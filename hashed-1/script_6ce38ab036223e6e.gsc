...
function function_e6937bfa()
{
	...
	while(a_ai_zombies.size > 0 || level.zombie_total > 0)
	{
		if(a_ai_zombies.size <= 3)
		{
			...
			{
				ai_zombie thread function_8a6addc9();
			}
		}
		wait(1);
		a_ai_zombies = zombie_utility::get_round_enemy_array();
...
*/
private function function_8a6addc9()
{
...
function round_think(restart = 0)
{
	...
	namespace_f551babc::function_74872db6();
	...
	while(true)
	{
		...
		level.first_round = 0;
		namespace_f551babc::function_189f87c1();
		callback::callback(#"hash_193ded5c8932fe29");
		level notify(#"end_of_round");
		bb::function_22b9bc7("end_of_round");
		uploadstats();
		if(!namespace_f551babc::function_b47f6aba() || !zm_utility::is_standard())
		{
		...
		foreach(player in players)
		{
			...
			if(!(isdefined(namespace_59ff1d6c::function_901b751c(#"hash_54dfa988db5db24c")) && namespace_59ff1d6c::function_901b751c(#"hash_54dfa988db5db24c")) && !player laststand::player_is_in_laststand() && (isdefined(player.heal.enabled) && player.heal.enabled))
			{
